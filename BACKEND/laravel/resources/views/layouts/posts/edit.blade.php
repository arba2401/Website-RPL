...
<form action="/dashboard/posts/{{ $post->slug }}" method="post" class="ijo-booklend" enctype="multipart/form-data">
    @method('put')
    @csrf
    ...
    <label for="author" class="form-label fw-bold" >Penulis</label>
                <input type="text" name="author" id="author" class="form-control @error('author') is-invalid @enderror" required autofocus value="{{ old('author', $post->author) }}">
    @error('author')
        <div class="invalid-feedback">
        {{ $message }}
        </div>
    ...
    <label for="pub_year" class="form-label fw-bold" >Tahun terbit</label>
                <input type="type" name="pub_year" id="pub_year" class="form-control @error('pub_year') is-invalid @enderror" required autofocus value="{{ old('pub_year', $post->pub_year) }}">
    @error('pub_year')
        <div class="invalid-feedback">
        {{ $message }}
        </div>
    ...
    <button type="submit" class="bg-ijo-booklend text-white rounded p-2 px-3 btn-bg-pink-hover border-ijo-booklend border-pink-hover mt-3">Edit</button>
