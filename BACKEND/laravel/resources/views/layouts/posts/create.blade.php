...
<div class="container">
    ...
    <div class="row mx-3">
        <div class="col">
            <form action="/dashboard/posts" method="post" class="ijo-booklend" enctype="multipart/form-data">
                @csrf
                ...
                <input type="text" name="author" id="author" class="form-control @error('author') is-invalid @enderror" required autofocus value="{{ old('author') }}">
                @error('author')
                    <div class="invalid-feedback">
                        {{ $message }}
                    </div>
                @enderror
                <label for="pub_year" class="form-label fw-bold" >Tahun terbit</label>
                <input type="type" name="pub_year" id="pub_year" class="form-control @error('pub_year') is-invalid @enderror" required autofocus value="{{ old('pub_year') }}">
                @error('pub_year')
                    <div class="invalid-feedback">
                        {{ $message }}
                    </div>
                ...
                @enderror
                <h2 class="ijo-booklend fw-bold fs-5">Deskripsi</h2>
                ...
                <button type="submit" class="bg-ijo-booklend text-white rounded p-2 px-3 btn-bg-pink-hover border-ijo-booklend border-pink-hover mt-3">Create</button>
                {{-- <input type="submit" value="Daftar" class="bg-ijo-booklend text-white rounded p-2 px-3 btn-bg-pink-hover border-ijo-booklend border-pink-hover mt-3"> --}}
